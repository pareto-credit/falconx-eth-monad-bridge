...
import { OfficialLedgerSigner } from '../script/officialLedgerSigner'
...
async function getTransactionSigner(hre: HardhatRuntimeEnvironment) {
    const ledgerAddress = process.env.LEDGER_ADDRESS

    // Prefer explicit software signers only when they are actually configured.
    if (process.env.PRIVATE_KEY || process.env.MNEMONIC) {
        const configuredSigners = await hre.ethers.getSigners()
    ...
    if (!ledgerAddress) {
        throw new Error(
            'No transaction signer configured. Set PRIVATE_KEY or MNEMONIC for raw Hardhat tasks, or set LEDGER_ADDRESS to use the official Ledger signer.'
        )
    }

    const derivationPath = process.env.LEDGER_DERIVATION_PATH || DEFAULT_LEDGER_DERIVATION_PATH
    const signer = new OfficialLedgerSigner(hre.ethers.provider, derivationPath)
    const actualAddress = await signer.getAddress()

    if (actualAddress.toLowerCase() !== ledgerAddress.toLowerCase()) {
        throw new Error(
            `Connected Ledger address ${actualAddress} does not match LEDGER_ADDRESS ${ledgerAddress}. Check LEDGER_DERIVATION_PATH.`
        )
    }
...
export async function sendEvm(
    ...
    const signer = await getTransactionSigner(srcEidHre)
    const provider = srcEidHre.ethers.provider
    ...
    // now attach
    const oft = new Contract(wrapperAddress, oftArtifact.abi, signer)
    ...
    try {
        ...
        if (approvalRequired) {
            ...
            const currentAllowance = await erc20.allowance(signerAddress, wrapperAddress)
    ...
    try {
        tx = await oft.send(sendParam, msgFee, signerAddress, {
